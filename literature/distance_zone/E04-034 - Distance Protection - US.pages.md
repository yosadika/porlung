# Page index: E04-034 - Distance Protection - US.pdf

Source PDF: E04-034 - Distance Protection - US.pdf
...
1
220X103
I = = 14113 A - Relay R setting > 14113 A
F1 1
√3X(S+4)
ZS=10Ω
Z =4Ω
...
1
3
I = 220X10 = 12702 A - Relay R setting > 12702 A
F2 1
√3X10
...
L
And
Z – electrical system source impedance behind the relay points
S
Z – electrical line impedance equivalent to protection relay reach setting
L
4
...
RELATIONSHIP BETWEEN RELAY VOLTAGE AND Z /Z RATIO
S L
...
system. Point R corresponds to the relay position; I and V are the current and voltage
R R
sensed by the relay.
The impedances Z and Z are depicted as source and transmission line impedances
s L
due to their location relative to the relay. Source impedance Z s is a quantity of the short
...
Line impedance Z is a quantity of the impedance of the protected region. The voltage
L
V utilized to the protection relay is I Z . The short circuit current at the reach point,
R R L
may be conveyed in terms of source to transmission line impedance ratio Z /Z using
S L
...
The universal relationship between V R and Z S /Z L , presented in Figure 5, is valid for all
cases of short circuit currents provided that few elementary rules are kept. These
rules are:
- For line faults, V is the line-line source voltage and Z /Z is the positive
∆ S L
sequence source to transmission line impedance ratio. V and I are the
R R
line-line protection relay voltage and current respectively.
V = V /(1+ (Z /Z )) (2)
R ∆ S L
- For ground short circuit currents, V is the line-neutral source voltage and
l-n
Z /Z is a composite ratio representing the positive and zero sequence
S L
impedances. V is the line-neutral protection relay voltage and I is the
R R
protection relay current for the faulted line.
V R = ( ZS)( 2 1 + p )+1 V 1-n (3)
ZL 2+q
Where:
)detar
%(
...
terms of an equivalent maximum Z S /Z L or S.I.R. Distance protection relays are
...
R - distance device at A
AZ<
R – directional device at A
AD
...
RD at A will hold due to current I F1 . At the same time, the impedance element is
...
the current through the protection relay from I F1 to I F2 while opening C, could end up
...
can be found using a phase comparator circuit which compares input signals S and
2
S 1, and performs whenever S 2 lags S 1 by 90° to 270°, as presented in the voltage
graph of Figure 9.
18
...
Z - impedance setting of the zone
n
X
V=IZ0 Restrain
ZN
...
characteristics. Decrease of load impedance from Z to Z will relate to the same
D3 D1
...
plane that is frequented by load (Z -Z -Z -Z ).
A B C D
...
X
R
Figure 12. Fully cross-polarized mho relay characteristic with variations of Z /Z
S L
ratio
25
...
where Z D1 , Z D2 , and Z D3 are the minimum load impedances allowed when lenticular,
