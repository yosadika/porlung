...
- Jika baris `line_impedance` yang terpilih cocok dengan filter tetapi nilai Z1/Z0 kosong atau tidak valid, aplikasi menampilkan alert ramah yang menjelaskan baris/kolom yang perlu dicek dan tetap membuka input manual; kondisi ini tidak boleh tampil sebagai traceback `st.exception` "Gagal membaca database spreadsheet".
...
- Marker fault: crosshair presisi, span pengapit di-highlight merah, label permanen menampilkan: sumber (remap tampilan `de` → `"Double-Ended Calculation"` hanya di DivIcon), jarak dari GI Lokal, jarak dari GI Remote (`line_length_km - distance_km`), nomor tower terdekat (regex `#\d+` dari kolom SPAN). Label `display:inline-block` + `white-space:nowrap` → lebar menyesuaikan konten, bukan fixed `icon_size`. Tanpa pointer/panah arah. Posisi adaptif via `fault_label_anchor_from_segment` (`icon_size=(200,90)`).
...
- Tombol `Simpan Case ke Cloud` di sidebar dan Setup DB berfungsi sebagai **general save**: menyimpan case ke `saved_cases`/`saved_cases_data`, upsert dataset penyebab ke `fault_cause`, dan upsert kalibrasi lokasi ke `fault_location` bila hasil Double-End sudah tersedia. Jika Double-End belum dihitung, bagian lokasi dilewati dengan pesan status; penyimpanan case dan dataset penyebab tetap diproses.
...
- **Simpan/Muat Case via Spreadsheet** (`cloud_cases.py`, di Setup DB > Case Storage): ZIP case → base64 **chunked** ke sheet `saved_cases_data` (1 baris/case, payload dipecah ≤49000 char/sel), indeks ringkas ke sheet `saved_cases` (upsert by `case_id = sha1(line_name|fault_time_cfg)`) termasuk metadata filter UPT/ULTG Local/Remote dan Segment. Daftar case dimuat **diurut `saved_at` terbaru**; pilih → rakit chunk → `restore_case_archive`; metadata indeks dipakai sebagai fallback restore filter untuk case lama. **Loader tersedia di sidebar "Case Storage"** (di atas ZIP uploader, muncul jika `database_spreadsheet_url` ada di session_state — bukan dari secrets saja) sehingga user bisa memilih case tersimpan **di landing page tanpa upload COMTRADE/ZIP**; tombol save tersedia di sidebar dan Setup DB. Tombol `Simpan Case ke Cloud` adalah general save: selain case, tombol ini juga menulis dataset ML penyebab/lokasi sebagaimana dijelaskan di bagian Machine Learning. **TANPA Google Drive** — service account akun personal tidak punya kuota Drive (`storageQuotaExceeded`), jadi payload disimpan langsung di spreadsheet. **Melengkapi** (tidak menggantikan) save/load ZIP manual.
- **Shareable link** (`?case=<16char_id>`): dihasilkan oleh `_render_share_link_widget()` di sidebar expander "Bagikan Link Case" dan tab Setup DB. Format pesan formal berisi nama case, saluran, waktu fault, dan URL. Bila `DATABASE_SPREADSHEET_URL` tidak ada di Streamlit secrets, tambahkan `?db=<spreadsheet_id>` ke link. Auto-load: saat URL berisi `?case=`, app memanggil `load_case_from_cloud` dengan `_creds_db_url` (termasuk secrets). Manual load via sidebar/Setup DB memanggil `st.query_params.clear()` sebelum `st.rerun()` agar `?case=` lama tidak menimpa case baru. **Display sidebar/landing-page** memakai `session_state["database_spreadsheet_url"]`, bukan `_creds_db_url` — mencegah daftar case muncul tanpa credentials eksplisit (secrets tidak memicu tampilan).
