...
| `tabs/line_parameter.py` | Render tab Line; **satu-satunya** selector sumber panjang line (`line_length_source`); simpan `effective_line_param` ke session_state; `@st.fragment`; Normalize sukses wajib `st.rerun(scope="app")`; baris spreadsheet dengan Z1/Z0 kosong tampil sebagai alert ramah, bukan traceback |
...
- **Line Parameter missing impedance UX:** jika baris `line_impedance` terpilih tetapi Z1/Z0 tidak dapat dibaca, tangkap `ValueError` dari `extract_impedance_from_row()` di UI Line Parameter dan tampilkan alert nyaman berisi baris terpilih, detail singkat, serta kolom impedansi terdeteksi. Jangan biarkan exception ini naik ke handler "Gagal membaca database spreadsheet" atau `st.exception`.
...
- **General cloud save:** tombol `Simpan Case ke Cloud` di sidebar dan Setup DB harus memanggil helper general save yang menyimpan case ke `saved_cases`/`saved_cases_data`, upsert `Machine Learning > Dataset Penyebab` ke `fault_cause`, dan upsert `Machine Learning > Kalibrasi Lokasi` ke `fault_location` bila `two_ended_result` tersedia. Jika DE belum dihitung, lokasi boleh dilewati dengan pesan status; jangan membuat kegagalan lokasi membatalkan penyimpanan case/dataset penyebab yang valid.
...
- **Tower Map label fault location:** label DivIcon menampilkan jarak dari GI Lokal + GI Remote + nomor tower terdekat (`#XXXX` diekstrak via `re.search(r'#\d+', span_text)`). Remap tampilan sumber: `'de'` → `'Double-Ended Calculation'` hanya di DivIcon (popup/selectbox tidak berubah). `display:inline-block` wajib agar lebar label menyesuaikan konten; tanpa itu label melebar ke 210px penuh. `fault_label_anchor_from_segment` dikalibrasi untuk `icon_size=(200,90)`. **Jangan tambahkan teks SPAN panjang** ke label DivIcon — cukup `#XXXX`; teks panjang melebar menutup tower marker tetangga.
- **Tower Schedule runtime keys cleared on restore:** `restore_case_archive` selalu menghapus `_TOWER_SCHEDULE_RUNTIME_KEYS` (`tower_schedule_df`, `tower_schedule_filtered_df`, `tower_schedule_selected_length_km`, `tower_schedule_selected_length_source`, `tower_schedule_loaded`) **sebelum** loop restore. Ini mencegah data tower case lama meracuni case baru yang tidak punya tower schedule. Jika case baru punya tower data, restore loop mengisi kembali key tersebut dari snapshot.
- **`_sync_checkbox` KeyError guard:** callback `on_change` di `fault_workflow_helpers._sync_checkbox` harus cek `if src_key in st.session_state` sebelum membaca nilai. Tanpa guard, `st.rerun()` dari sidebar (sebelum tab Local/Remote End dirender) crash karena widget `local_fc_use_auto_fault_detection` belum ada di session_state.
- **`st.query_params.clear()` sebelum `st.rerun()` pada manual load:** semua titik yang melakukan load case secara manual (sidebar cloud, ZIP upload, landing page cloud, Setup DB cloud) wajib memanggil `st.query_params.clear()` sebelum `st.rerun()`. Tanpa ini, `?case=<old_id>` di URL memicu auto-load kembali dan menimpa case yang baru dipilih.
- **Display cloud case list vs API call:** kondisi tampil sidebar/landing-page cloud case list memakai `st.session_state.get("database_spreadsheet_url")` — hanya muncul jika user aktif menyediakan URL (upload credentials atau buka via `?db=`). `_creds_db_url` (termasuk Streamlit secrets) dipakai HANYA untuk API call aktual. Jangan kembalikan kondisi tampil ke `_creds_db_url` — akan memunculkan daftar case tanpa credentials.
