...
- **Base setting:** untuk `line_data`, base `primary` wajib memakai kolom `Prim`; base `secondary` wajib memakai kolom `Sec` lalu dikonversi memakai CT/VT Signal Assignment aktif. Rasio VT/CT di spreadsheet adalah metadata/audit, bukan pengganti Signal Assignment.
